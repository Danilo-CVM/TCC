...
### Cálculo do Beta
A volatilidade que uma carteira de referência (para o presente caso o iBovespa) ou uma carteira de mercado exibe é conhecida como risco sistemático. Beta é a medida histórica de risco de qualquer ação ou carteira individual em relação ao risco da carteira de mercado. Em outras palavras, ele mede a volatilidade de qualquer título em relação à volatilidade geral do mercado.
...
De posse dessas informações foi gerado um banco de dados contendo a performance semana a semana de cada um dos ativos em diversas datas diferentes.
...
Etapa final do projeto que rcebeu como insumos os resultados das etapas anteriores sendo aplicados algoritmos genéticos no intuito de se chegar a uma alocação que aposta em ativos que vêm apresentando bom desempenho em períodos anteriores balanceando-os na intenção de se manter o Beta dentro de uma faixa definida para se chegar a uma carteira que promete bom desempenho, sem que o risco supere o limite previamente definido.
Apesar de ter rodado o algoritmo python que calcula a rentabilidade dos ativos para um período extenso, devido ao grande número de ativos que compõem o índice bovespa, a cada "Not a Number(NaN)" recebido do servidor da yahoo correspondia a eliminação daquela semana específica, pois para podermos comparar o índice com a carteira rebalanceada, era necessário que todos as semanas estudadas contivessem dados de todos os ativos. Por esse motivo no primeiro teste só conseguimos aproveitar 7 semanas para a avaliação do projeto. Foram feitos testes com essas poucas semanas para ter uma primeira idéia se o caminho seguido tinha boas perspectivas. Os resultados conseguidos nesse primeiro teste foram animadores e constam da aba de resultados do arquivo "planilha" do branch "Genetic". 
...
A tabela acima, assim como o gráfico de performance podem ser encontrados na aba "Resultados" de ambas as planilhas constantes do branch "Genetic".

## Conclusões

Do presente projeto pôde-se concluir que os algoritmos de machine learning são boas ferramentas para a previsão de tendências de movimentos de ações e índices e por isso merecem ser estudadas a fundo a fim de melhorar sua acurácia e ampliar sua gama de utilizações. Em especial a técnica Random Forest se mostrou extremamente potente, mesmo sendo uma das mais simples e com menor custo computacional.
